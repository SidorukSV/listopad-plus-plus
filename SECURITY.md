...
Представление системного монитора добавляет к этой поверхности системный PDH:
бинарный `.blg` разбирает `pdh.dll`, а не собственный код. Чтение остаётся
локальным и внутрипроцессным, выполняется в обычном непривилегированном
процессе на рабочем потоке, не запускает внешних программ, не обращается к
сети и не изменяет исходный файл. Открывать журналы счётчиков из недоверенных
источников следует с тем же вниманием, что и любые другие чужие файлы.
