...
Если подписанный broker недоступен, пользователь может явно согласиться на
перезапуск того же `ListopadPP.exe` через `runas`. В этом fallback-режиме весь
редактор работает с повышенными правами.
...
Перед fallback-перезапуском текущий сеанс и dirty-текст фиксируются через
`SessionStore`. Elevated-процесс ждёт завершения прежнего single instance,
восстанавливает вкладки и повторяет только сохранение с совпавшими ID вкладки,
каноническим путём и SHA-256 закодированных байтов.
